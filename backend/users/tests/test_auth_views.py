...
class ForgotPasswordCase(TestCase):
    @classmethod
    ...
    def test_user_does_not_exist(self):
        ...
        test_results.append(["ForgotPasswordCase", "test_user_does_not_exist", response.status_code, response.json()])
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.json()["message"], "Username or password is incorrect")
    ...
    def test_user_does_not_active(self):
        ...
        test_results.append(["ForgotPasswordCase", "test_user_does_not_active", response.status_code, response.json()])
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.json()["message"], "User not allowed to do it")
    ...
    def test_user_has_default_password(self):
        ...
        test_results.append(["ForgotPasswordCase", "test_user_has_default_password", response.status_code, response.json()])
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.json()["message"], "User not allowed to do it")
        
    def test_answer_does_not_match(self):
        ...
        self.assertEqual(response.json()["message"], "The answer does not match")
        
    def forgot_password_successfully(self):
        ...
        test_results.append(["ForgotPasswordCase", "forgot_password_successfully", response.status_code, response.json()])
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["message"], "Reset password successfully")